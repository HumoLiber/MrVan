...
-- Створення політик безпеки (додамо IF NOT EXISTS)
DO $$
BEGIN
    -- Для таблиці users
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'users' AND policyname = 'Users can view and edit their own data'
    ) THEN
    ...
    -- Для таблиці user_documents
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'user_documents' AND policyname = 'Users can view and edit their own documents'
    ) THEN
    ...
    -- Для таблиці vehicles
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'vehicles' AND policyname = 'Users can view and edit their own vehicles'
    ) THEN
    ...
    -- Для таблиці agreements
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'agreements' AND policyname = 'Users can view and edit their own agreements'
    ) THEN
